...
function fmtDim(v) {
  // length/width: no .00, hide zeros
  return fmtNum(v, { max: 2, min: 0, blankZero: true });
}

function buildDeliveryHtml(doc) {
  // ✅ support both
  const docNumber = pick(doc, ["requestNumber", "invoiceNumber"], "");
...
export default function DeliveryNoteModal({ open, onClose, doc }) {
  ...
  const modalTitle =
    doc?.requestNumber || doc?.invoiceNumber || "Delivery Note";

  const srcDoc = useMemo(() => buildDeliveryHtml(doc || {}), [
    doc?.requestNumber,
    doc?.invoiceNumber,
    doc,
  ]);
      ...
      <div className="delivery-note-modal" onMouseDown={(e) => e.stopPropagation()}>
        <div className="delivery-note-header">
          ...
          <div className="delivery-note-actions">
            ...
            <button className="delivery-note-btn delivery-note-danger" onClick={onClose}>
              Close
            </button>
          </div>
        </div>
