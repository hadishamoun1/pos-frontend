...
// ✅ Convert logo to base64 so it renders correctly inside iframe srcDoc
...
function buildDeliveryHtml(doc, isRevo, logoBase64) {
  ...
  const items = doc?.details || doc?.items || doc?.invoiceItems || doc?.lines || doc?.rows || [];
  ...
  const rows = (Array.isArray(items) ? items : []).map((it, idx) => {
    ...
    return { itemNumber, itemName, unit, box, sheet, length, width, sqm };
  });

  const rowsHtml = rows.map((r) => {
    const unitLower = String(r.unit || "").toLowerCase().trim();
    const isSheetUnit = unitLower === "sheet" || unitLower.includes("sheet");
    const boxText = isSheetUnit ? "" : fmtNum(r.box, { max: 2, min: 0, blankZero: true });
  ...
  // ✅ Revo header — only injected when isRevo, Shamoun gets nothing
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    body { margin: 0; font-family: Arial, sans-serif; color: #111; background: #ececec; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .delivery-note-page { width: 210mm; min-height: 297mm; margin: 12px auto; background: #fff; padding: 12mm; box-shadow: 0 10px 30px rgba(0,0,0,0.18); border-radius: 2mm; }
    .delivery-note-title { font-size: 20px; font-weight: 800; margin-bottom: 6px; }
    .delivery-note-muted { color:#666; margin-bottom: 10px; }
    .delivery-note-headerBox { border: 1px solid #ddd; border-radius: 8px; padding: 12px; margin-bottom: 14px; }
    .delivery-note-top { display: flex; justify-content: space-between; gap: 16px; align-items: flex-start; }
    .delivery-note-leftMeta { flex: 1; font-size: 13px; line-height: 1.7; }
    .delivery-note-rightMeta { min-width: 220px; text-align: right; font-size: 13px; line-height: 1.7; }
    .delivery-note-rowLine b { display:inline-block; min-width: 95px; }
    ...
    @media print {
      body { background: #fff; }
      .delivery-note-page { width: auto; min-height: auto; margin: 0; padding: 0; box-shadow: none; border-radius: 0; }
    }
  </style>
</head>
<body>
  <div class="delivery-note-page">
    ...
    <table>
      <thead>
        <tr>
          <th>Item No</th><th>Item Name</th><th>Box</th><th>Sheet</th>
          <th>Length</th><th>Width</th><th>SQM</th><th>Unit</th>
        </tr>
      </thead>
      <tbody>
        ${rowsHtml || `<tr><td colspan="8" class="delivery-note-table-muted">No items</td></tr>`}
      </tbody>
    </table>
  </div>
</body>
...
export default function DeliveryNoteModal({ open, onClose, doc }) {
  ...
  const srcDoc = useMemo(
    () => buildDeliveryHtml(doc || {}, isRevo, logoBase64),
  ...
  return (
    <div className="delivery-note-overlay" onMouseDown={onClose}>
      <div className="delivery-note-modal" onMouseDown={(e) => e.stopPropagation()}>
        <div className="delivery-note-header">
          <div className="delivery-note-title">{modalTitle}</div>
          <div className="delivery-note-actions">
            <button className="delivery-note-btn" onClick={handlePrint}>Print</button>
            <button className="delivery-note-btn delivery-note-danger" onClick={onClose}>Close</button>
          </div>
        </div>
        <iframe
          ref={iframeRef}
