...
const ProfitabilityReport = () => {
  ...
  // ✅ Format the unit type + sheetsPerBox into a single readable string
  // e.g. "Box (6/box)", "SQM", "Sheet", "Unit"
    ...
    <div className="profitability-report">
      ...
      <div className="profitability-report__filters">
        <div className="profitability-report__filter-row">
          ...
          <button onClick={loadReport} className="profitability-report__btn-apply">Apply Filters</button>
                ...
                <tbody>
                  {data.rows.map((row, idx) => (
                    <tr key={idx} className={row.invoiceType === 'RTN' ? 'profitability-report__table-row--return' : ''}>
                      ...
                      <td>{row.origin}</td>
                      <td>                                                      {/* ✅ NEW */}
                        <span className={`profitability-report__unit-badge profitability-report__unit-badge--${(row.unitType || 'sqm').toLowerCase()}`}>
                          {fmtUnitType(row.unitType, row.sheetsPerBox)}
                        </span>
                      </td>
                      <td>{row.quantity}</td>
                  ...
                  <div className="profitability-report__customer-header" onClick={() => toggleCustomer(customer.customerId)}>
                    ...
                    <div className="profitability-report__customer-totals">
                      ...
                      <div className="profitability-report__total-item">
                        <span>Profit:</span>
                        <strong className={customer.profit < 0 ? 'profitability-report__total-item--negative' : 'profitability-report__total-item--positive'}>${fmt(customer.profit)}</strong>
                      </div>
                      <div className="profitability-report__total-item">
                        <span>Margin:</span>
                        <strong className={customer.margin < 0 ? 'profitability-report__total-item--negative' : 'profitability-report__total-item--positive'}>{customer.margin.toFixed(2)}%</strong>
                      </div>
                    </div>
                  </div>
                        ...
                        <tbody>
                          {customer.items.map((item) => (
                            <tr key={item.itemVariantId}>
                              ...
                              <td>{item.origin}</td>
                              <td>                                              {/* ✅ NEW */}
                                <span className={`profitability-report__unit-badge profitability-report__unit-badge--${(item.unitType || 'sqm').toLowerCase()}`}>
                                  {fmtUnitType(item.unitType, item.sheetsPerBox)}
                                </span>
                              </td>
                              ...
                              <td className={`profitability-report__amount ${item.profit < 0 ? 'profitability-report__amount--negative' : 'profitability-report__amount--positive'}`}>${fmt(item.profit)}</td>
                              <td className={item.margin < 0 ? 'profitability-report__amount--negative' : 'profitability-report__amount--positive'}>{item.margin.toFixed(2)}%</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                <div key={item.itemVariantId} className="profitability-report__item-group">
                  <div className="profitability-report__item-header" onClick={() => toggleItem(item.itemVariantId)}>
                    <div className="profitability-report__item-info">
                      ...
                      <div className="profitability-report__item-details">
                        <div className="profitability-report__item-name-row">
                          ...
                          {/* ✅ NEW: unit type badge inline with name */}
                          <span className={`profitability-report__unit-badge profitability-report__unit-badge--${(item.unitType || 'sqm').toLowerCase()}`}>
                            {fmtUnitType(item.unitType, item.sheetsPerBox)}
                          </span>
                        </div>
                    ...
                    <div className="profitability-report__item-totals">
                      ...
                      <div className="profitability-report__total-item">
                        <span>Profit:</span>
                        <strong className={item.profit < 0 ? 'profitability-report__total-item--negative' : 'profitability-report__total-item--positive'}>${fmt(item.profit)}</strong>
                      </div>
                      <div className="profitability-report__total-item">
                        <span>Margin:</span>
                        <strong className={item.margin < 0 ? 'profitability-report__total-item--negative' : 'profitability-report__total-item--positive'}>{item.margin.toFixed(2)}%</strong>
                      </div>
                    </div>
                  </div>
                        ...
                        <tbody>
                          {item.customers.map((cust) => (
                            <tr key={cust.customerId}>
                              ...
                              <td className={`profitability-report__amount ${cust.profit < 0 ? 'profitability-report__amount--negative' : 'profitability-report__amount--positive'}`}>${fmt(cust.profit)}</td>
                              <td className={cust.margin < 0 ? 'profitability-report__amount--negative' : 'profitability-report__amount--positive'}>{cust.margin.toFixed(2)}%</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                <div key={invoice.invoiceId} className="profitability-report__invoice-group">
                  ...
                  >
                    ...
                    <div className="profitability-report__invoice-totals">
                      ...
                      <div className="profitability-report__total-item">
                        <span>Profit:</span>
                        <strong className={invoice.profit < 0 ? 'profitability-report__total-item--negative' : 'profitability-report__total-item--positive'}>${fmt(invoice.profit)}</strong>
                      </div>
                      <div className="profitability-report__total-item">
                        <span>Margin:</span>
                        <strong className={invoice.margin < 0 ? 'profitability-report__total-item--negative' : 'profitability-report__total-item--positive'}>{invoice.margin.toFixed(2)}%</strong>
                      </div>
                    </div>
                  </div>
                      ...
                      <table className="profitability-report__items-table">
                        ...
                        <tfoot>
                          <tr className="profitability-report__totals-row">
                            ...
                            <td className={`profitability-report__amount ${invoice.profit < 0 ? 'profitability-report__amount--negative' : 'profitability-report__amount--positive'}`}><strong>${fmt(invoice.profit)}</strong></td>
                            <td className={invoice.margin < 0 ? 'profitability-report__amount--negative' : 'profitability-report__amount--positive'}><strong>{invoice.margin.toFixed(2)}%</strong></td>
                          </tr>
                        </tfoot>
                      </table>
                    </div>
